...
def position_limit_excursions(validator, dense_times, dense_path):
    """Per joint outside its limits: by how much at worst, and when.

    The joint names alone cannot say whether a path leaves a limit throughout
    or grazes it for one sample: a 171 s section once failed on wrist_3 at
    0.05 mrad past -2*pi at its final instant, and only the time and amount
    show a section ending slightly earlier would pass.
    """
    lower, upper = validator.robot.qlim
    ...
    out = {}
    for index, name in enumerate(JOINT_NAMES):
        excess = np.maximum(lower[index] - dense_path[:, index],
                            dense_path[:, index] - upper[index])
        outside = np.flatnonzero(excess > 1e-9)
        if len(outside):
            out[name] = {'max_excursion': float(excess.max()),
                         'first_time_s': float(dense_times[outside[0]]),
                         'last_time_s': float(dense_times[outside[-1]]),
                         'samples': int(len(outside))}
    return out
...
def validate(validator, path, waypoint_times, positions, quaternions,
    ...
    report = {
        ...
        'position_limit_excursions': position_limit_excursions(
            validator, dense_times, dense_path),
...
def validate_route(validator, route, velocity_limits=None):
    """Every segment of a warmup route, by the rules one warmup is held to.

    A route is refused if any segment fails, and the report names which one:
    a caller told only that "the warmup failed" cannot tell a blocked leg out
    of home from a blocked leg into the task start.
    """
    reports, failures = [], []
    for index, segment in enumerate(route.get('segments', [])):
        report = validate_warmup(validator, segment, velocity_limits)
        ...
        if not report['self_clearance']['passed']:
            reasons.append('self-clearance '
                           f"{report['self_clearance']['min_distance_m'] * 1000:.1f} mm "
                           'below the floor')
        prefix = ('' if len(route['segments']) == 1
                  else f'segment {index + 1} of {len(route["segments"])}: ')
        failures.append(prefix + ('; '.join(reasons) or 'failed validation'))
    return {'command': 'warmup_route',
            'segments': reports,
            'segment_count': len(reports),
            'failed_segments': [i for i, r in enumerate(reports) if not r['passed']],
            'failures': failures,
