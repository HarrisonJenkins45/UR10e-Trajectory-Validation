...
Certification for hardware needs a chosen UR execution controller and payload
configuration, and the rail's actual hardware data. The command validator
currently rejects acceleration and jerk above these provisional or assumed
numbers. That is a planning policy, not evidence that the hardware can safely
execute every command below them.
"""
import numpy as np
