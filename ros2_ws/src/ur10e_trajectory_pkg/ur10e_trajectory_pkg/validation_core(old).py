        ...
        def try_point(target_pos, seed_arm, prev_arm, check_jump):
            ...
            for attempt in range(max_attempts + 1):
                ...
                if check_jump:
                    ...
                    jump = False
                collide = self.check_all_collisions_manual(q_full, verbose=True)
...
#         reachable/safe space) and return the maximal contiguous runs of
#         waypoints that are each individually feasible: IK converges, no
#         self/environment collision, manipulability above threshold, and no
#         excessive joint velocity relative to the PREVIOUS point in the same
#         run. Only runs of length >= min_length are returned.
...
#                 q_full = np.concatenate(([rail_pos], q_arm))
#                 J = self.compute_jacobian(q_arm)
#                 w = np.sqrt(np.abs(np.linalg.det(J @ J.T)))
#                 low_manip = w < manipulability_threshold
#                 if check_jump:
#                     joint_vel = np.abs(q_arm - prev_arm) / dt_waypoint
#                     jump = np.any(joint_vel > max_joint_vel_threshold)
#                 else:
#                     jump = False
#                 collide = self.check_all_collisions(q_full)
#                 if not (low_manip or jump or collide):
#                     return True, q_arm, q_full
#                 search_radius += 0.05
#             return False, None, None
...
#                 joint_vel = np.abs(q_arm - prev_config_arm) / dt_waypoint
#                 jump = np.any(joint_vel > max_joint_vel_threshold)
#                 collide = self.check_all_collisions(q_full) if sol.success else False
#                 return q_arm, q_full, sol, w, low_manip, jump, collide, joint_vel

#             q_sol_arm, q_sol_full, sol, w, low_manip, jump, collide, joint_vel = try_solve(prev_config_arm)
...
#                 if verbose:
#                     print(f'[waypoint {k}] attempt {attempt}: collide={collide} jump={jump} '
#                           f'low_manip={low_manip} (w={w:.4f}) ik_ok={sol.success} -- searching locally (radius={search_radius:.2f})...')
#                 random_offset = (2 * np.random.rand(6) - 1) * search_radius
#                 local_guess = prev_config_arm + random_offset
#                 q_sol_arm, q_sol_full, sol, w, low_manip, jump, collide, joint_vel = try_solve(local_guess)
#                 search_radius += 0.05
#                 attempt += 1
...
#             if low_manip:
#                 msg = f'Singularity at waypoint {k}: manipulability w={w:.5f} < {manipulability_threshold} (after {attempt - 1} recovery attempts)'
#                 if verbose:
#                     print(f'[waypoint {k}] {msg} | q={np.round(q_sol_full, 3)}')
#                 return False, np.array([]), np.array([]), msg
