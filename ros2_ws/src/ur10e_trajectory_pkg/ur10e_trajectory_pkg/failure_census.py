...
def recording_record(csv_path=None):
    """The recording a stage loaded: its path, and the digest of its bytes."""
    from ur10e_trajectory_pkg.ClientNode import DEFAULT_CSV_PATH

    path = str(csv_path or DEFAULT_CSV_PATH)
    return {'csv_path': path, 'csv_sha256': file_digest(path)}


def recording_mismatch(recorded, current):
    """Why an upstream artifact was built from other bytes, or None if it was not.

    The stages are separate commands and each loads the recording itself, so
    a graph built against one recording on candidates generated from another
    would still plan -- against targets nobody generated candidates for.

    Compared by digest, not path: the same recording is mounted at different
    paths in different containers, and a file can be replaced in place. A
    record carrying no digest predates --csv, when every stage loaded the
    packaged recording, so it agrees with those bytes and no others.
    """
    from ur10e_trajectory_pkg.ClientNode import DEFAULT_CSV_PATH
    ...
    expected, source = recorded.get('csv_sha256'), recorded.get('csv_path')
    if expected is None:
        expected = file_digest(DEFAULT_CSV_PATH)
        source = f'{DEFAULT_CSV_PATH} (no digest recorded, so the packaged one)'
...
def placement_RG_for(name, csv_path=None, start_index=0):
    ...
    placement_envelope.placement_transform applied to it, rotating about the
    first target. For a slice, the first pose is the slice's own.
    """
    from ur10e_trajectory_pkg import placement_envelope
