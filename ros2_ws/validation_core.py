...
- COLLISION BACKEND SWITCHED FROM roboticstoolbox/spatialgeometry
  (Shape.iscollided(), backed by python-fcl) TO pybullet.
  spatialgeometry's iscollided() was flagging a self/env "collision" on
  essentially every waypoint that visual inspection showed was clean. The
  most likely culprits are all in the fcl mesh path: fcl needs a *convex*
  mesh (or convex decomposition) for reliable mesh-mesh queries, and
  non-watertight/high-poly UR10e collision STLs loaded as single concave
  meshes are a classic source of spurious hits in fcl; on top of that,
  spatialgeometry's .pose assignment (see the old code below this comment
  block) depends on getting geom.base composed exactly right, which is a
  second place a subtle transform bug can silently produce false
  positives. Rather than debug fcl's mesh handling, this version hands the
  exact same URDF to pybullet and lets its own URDF loader + GJK/EPA
  narrowphase do the geometry work -- it natively respects each link's
  <collision><origin> offset, so there's no manual geom.pose composition
  left to get wrong. The kinematic-adjacency filtering logic below
  (_compute_anchor / _is_adjacent) is unchanged and is reused as-is,
  just applied to pybullet's link indices instead of roboticstoolbox's.
"""
import tempfile
import os
import numpy as np
    ...
    def find_feasible_segments(self, ee_x, ee_y, ee_z, ee_quat, q_seed, min_length,
            ...
            def try_point(target_pos, target_quat, seed_arm, prev_arm, check_jump):
                ...
                for attempt in range(max_attempts + 1):
                    ...
                    search_radius += 0.05
                    print(f"Attempt #{attempt} failed, checking a new IK solution...")
